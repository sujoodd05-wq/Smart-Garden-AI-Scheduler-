...
## Project Overview
The system is designed to automate the decision of "when" and "how" to water a garden. It consists of two primary engines developed from scratch without the use of external machine learning libraries:

1. **Perceptron Classifier**: A machine learning model that predicts whether a plant requires watering based on its features.
2. **Simulated Annealing Optimizer**: A metaheuristic algorithm that finds the most efficient watering sequence to minimize walking distance and penalties.
...
It uses the **Metropolis Criterion** to escape local optima and settle on a global minimum cost by balancing exploration and exploitation through a cooling schedule.
...
* **Data Format**: CSV for external datasets
...
2. **Run the MainGUI class** located in the source folder.
3. **Add plants** manually in the "Garden" tab or **upload a CSV file** in the "Perceptron" tab.
4. **Train the model** to see the Learning Curve and update plant status.
5. **Navigate to the SA Optimizer tab**, set the number of plants to visit, and run the optimizer.
6. **Use "Show SA Steps"** to view the animated path development process.
